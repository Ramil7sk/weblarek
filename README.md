...
`clear()` - очищает модальное окно.

События класса:
При нажатии на кнопку закрытия формы и за пределы модального окна генерируется событие `modal:close`.
...
Поля класса:
`buttonElement: HTMLButtonElement` - кнопка модального окна.
`itemsPrice: HTMLElement` - поле хранящее сумму совершнной покупки.

Методы класса:
`set total(value: number)` - устанавливает сумму совершенной покупки.

События класса:
При нажатии на кнопку "За новыми покупками" окно закрывается.
...
Поля класса:
`cartContent: HTMLElement` - список для отображения элементов товаров в корзине.
`cartButton: HTMLButtonElement` - кнопка для оформления заказа в корзине.
`cartPrice: HTMLElement` - стоимость товаров в корзине.
...
Поля класса:
`cardIndex: HTMLElement` - хранит иднек товара в корзине.
...
`products:changed` - добавление товаров в модель данных.
`card:selected` - изменение карточки в модальном окне. 
`payment:changed` - изменение способа оплаты.
`buyer:changed` - изменение состояния модели покупателя.
`buyer:changed` - генерирует единое событие об изменении модели

###### События при взаимодействии пользователя с интерфейсом 
`card:open` - выбор карточки для отображения в модальном окне
`selectedСard:basketAction` - изменение состояния товара в корзине
